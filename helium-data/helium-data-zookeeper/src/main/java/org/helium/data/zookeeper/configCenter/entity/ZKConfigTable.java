 ...
 */
public class ZKConfigTable<K, V extends ConfigTableItem>
{
	...
	public ZKConfigTable(String tableName, Hashtable<K, V> table, Date version, Class<K> keyType, Class<V> valueType)
	{
	...
	public ZKConfigTable(String name, Date version)
	{
		tableName = name;
		this.version = version;
	}
	...
	public void addEvent(final ConfigUpdateAction<ZKConfigTable<K, V>> updateCallback) throws ConfigurationException

	{
		ZKConfigurationManager.getInstance().callBackConfigData(keyType, valueType, this.getTableName(), updateCallback);
	}
}
