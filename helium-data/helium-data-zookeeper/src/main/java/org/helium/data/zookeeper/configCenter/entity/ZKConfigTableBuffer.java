 ...
 */
public class ZKConfigTableBuffer extends SuperPojo
{
	...
	public <K, V extends ConfigTableItem> ZKConfigTable<K, V> toTable(Class<K> keyType, Class<V> valueType) throws ConfigurationNotFoundException, ConfigurationFailedException
	{
		...
		ZKConfigTable<K, V> table = new ZKConfigTable<K, V>(this.getTableName(), innerTable, this.getVersion(), keyType, valueType);
		try {
			table.runAfterLoad();
		} catch (Exception e) {
