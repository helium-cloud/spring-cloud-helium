 ...
 */
public class RedisRouteDirector2 {
    private static final Logger LOGGER = LoggerFactory.getLogger(RedisRouteDirector2.class);
    private static RedisRouteDirector2 instance = null;
    private static Byte syncRoot = new Byte("0");

    private static Map<String, IRouteProvider2<CFG_RedisSentinels, Pool<Jedis>>> hashProvider =
            new HashMap<String, IRouteProvider2<CFG_RedisSentinels, Pool<Jedis>>>();

    private static Map<String, Properties> configs = new HashMap<String, Properties>();

    private RedisRouteDirector2() {
    }


    public static RedisRouteDirector2 getInstance() {
        if (instance == null)
            instance = new RedisRouteDirector2();
        return instance;
    }

    public IRouteProvider2<CFG_RedisSentinels, Pool<Jedis>> getRouteProvider(String rouleName) {
     ...
     * @return
     */
    public IRouteProvider2<CFG_RedisSentinels, Pool<Jedis>> initRouteProvider(String roleName, Properties prop) {
        updateConfig(roleName, prop);
        return getRouteProvider(roleName);
    }

    public IRouteProvider2<CFG_RedisSentinels, Pool<Jedis>> initRouteProvider(String roleName, List<CFG_RedisSentinels> configs) {
    ...
    private void updateConfig(String roleName, Properties prop) {
        ...
        List<CFG_RedisSentinels> cfgs = new ArrayList<>();
        for (int i = 0; i < serverNum; i++) {
            ...
            if (prop.containsKey(name)) {
                ...
                if (addrs == null || addrs.length() > 0) {
                    CFG_RedisSentinels sentinelNode = new CFG_RedisSentinels();
        ...
        if (cfgs.size() != serverNum)
            throw new RuntimeException(String.format(" RedisSentinelClient  rouleName=%s  cfgs.size() != serverNum ", roleName));

        hashProvider.put(roleName, new HashRouteProvider2(roleName, cfgs));

    }


    private void updateConfig(List<CFG_RedisSentinels> configs) {
        try {

            DictionaryList<String, CFG_RedisSentinels> list =
                    new DictionaryList<String, CFG_RedisSentinels>();
            list.fillWith(new ArrayList<CFG_RedisSentinels>(configs), i -> i.getRoleName());

            //Hashtable<String, IRouteProvider<CFG_RedisSentinels, Pool<Jedis>>> hash = new Hashtable<String, IRouteProvider<CFG_RedisSentinels, Pool<Jedis>>>();
            for (String role : list.keys()) {
                List<CFG_RedisSentinels> items = list.get(role);
                if (!hashProvider.containsKey(role)) {
                    hashProvider.put(role, new HashRouteProvider2(role, items));
                }

            }

            // hashProvider = hash;
            LOGGER.info("hashProvider loaded");
