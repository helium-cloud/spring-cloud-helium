...
abstract class PipelineBase extends Queable implements IBinaryRedisPipeline,
        IBasicRedisPipeline {
    ...
    public Response<byte[]> getrange2(byte[] key, long startOffset, long endOffset) {
        getClient(key).getrange(key, startOffset, endOffset);
        return getResponse(BuilderFactory.BYTE_ARRAY);
    }
    ...
    public Response<Map<String, byte[]>> hgetAllStringByteArray(byte[] key) {
        getClient(key).hgetAll(key);
        return getResponse(BuilderFactory.STRING_BYTEARRAY_MAP);
    }
