...
*/
	public static void closeSocket(OutputStream out) throws IOException
	{
		byte[] header;
...
* @param sock the Socket object
*/
	public static boolean activeTest(OutputStream out,InputStream in) throws IOException
	{
		byte[] header;
		header = packHeader(FDFS_PROTO_CMD_ACTIVE_TEST, 0, (byte)0);
		out.write(header);
		
		RecvHeaderInfo headerInfo = recvHeader(in, TRACKER_PROTO_CMD_RESP, 0);
...
*/
 ...
 {
 	byte[] bsFilename = remote_filename.getBytes(FastdfsRouteDirector.getCharset());
 	byte[] bsKey = secret_key.getBytes(FastdfsRouteDirector.getCharset());
 	byte[] bsTimestamp = (new Integer(ts)).toString().getBytes(FastdfsRouteDirector.getCharset());
