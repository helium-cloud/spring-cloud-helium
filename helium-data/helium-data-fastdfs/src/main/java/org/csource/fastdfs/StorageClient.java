...
public class StorageClient {
	 ...
	 * @param port the tracker server, can be null
	 * @param timeout the storage server, can be null
	 */
	public StorageClient(String host, int port, int timeout) {
		this.storageServer = new StorageServer(host, port, timeout);
	}
	...
	public StorageServer getConnection() {
		return storageServer;
	}

	public void setStoragePathIndex(byte index) {
		this.storageServer.setStore_path_index(index);
	}

	public void connect() throws IOException {
		if (storageServer != null)
			storageServer.getSocket();
	}
	...
	public String upload_file1(String master_file_id, String prefix_name, byte[] file_buff, String file_ext_name, NameValuePair[] meta_list)
		...
		parts = upload_file(parts[0], parts[1], prefix_name, file_buff, file_ext_name, meta_list);
		if (parts != null) {
	...
	protected String[] do_upload_file(byte cmd, String group_name, String master_filename,
		...
		try {
			...
			if (file_ext_name != null && file_ext_name.length() > 0) {
				byte[] bs = file_ext_name.getBytes(FastdfsRouteDirector.getCharset());
			...
			if (bUploadSlave) {
				masterFilenameBytes = master_filename.getBytes(FastdfsRouteDirector.getCharset());
			...
			if (bUploadSlave) {
				...
				byte[] bs = prefix_name.getBytes(FastdfsRouteDirector.getCharset());
	...
	protected int do_append_file(String group_name, String appender_filename,
		...
		try {
			...
			appenderFilenameBytes = appender_filename.getBytes(FastdfsRouteDirector.getCharset());
	...
	public NameValuePair[] get_metadata(String group_name, String remote_filename) throws IOException, MyException {
		...
		try {
			...
			return ProtoCommon.split_metadata(new String(pkgInfo.body, FastdfsRouteDirector.getCharset()));
		} catch (IOException ex) {
	...
	public int set_metadata(String group_name, String remote_filename,
		...
		try {
			...
			if (meta_list == null) {
				...
				meta_buff = ProtoCommon.pack_metadata(meta_list).getBytes(FastdfsRouteDirector.getCharset());
			}

			filenameBytes = remote_filename.getBytes(FastdfsRouteDirector.getCharset());
			sizeBytes = new byte[2 * ProtoCommon.FDFS_PROTO_PKG_LEN_SIZE];
			...
			bs = group_name.getBytes(FastdfsRouteDirector.getCharset());
