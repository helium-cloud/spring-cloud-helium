...
public class AbstractDynamicConfigurationTest {
    ...
    public void testConstants() {
        assertEquals("helium.config-center.", PARAM_NAME_PREFIX);
        assertEquals("helium.config-center.workers", DEFAULT_THREAD_POOL_PREFIX);
        assertEquals("helium.config-center.thread-pool.prefix", THREAD_POOL_PREFIX_PARAM_NAME);
        assertEquals("helium.config-center.thread-pool.size", THREAD_POOL_SIZE_PARAM_NAME);
        assertEquals("helium.config-center.thread-pool.keep-alive-time", THREAD_POOL_KEEP_ALIVE_TIME_PARAM_NAME);
        assertEquals(1, DEFAULT_THREAD_POOL_SIZE);
        assertEquals(60 * 1000, DEFAULT_THREAD_POOL_KEEP_ALIVE_TIME);

        // @since 2.7.8
        assertEquals("helium.config-center.group", GROUP_PARAM_NAME);
        assertEquals("helium.config-center.timeout", TIMEOUT_PARAM_NAME);
    }
