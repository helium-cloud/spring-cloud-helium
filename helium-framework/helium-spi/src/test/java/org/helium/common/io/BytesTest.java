...
public class BytesTest {
    ...
    public void testMain() throws Exception {
        ...
        String str = Bytes.bytes2base64("helium".getBytes());
        byte[] bytes = Bytes.base642bytes(str, 0, str.length());
        assertThat(bytes, is("helium".getBytes()));

        byte[] bytesWithC64 = Bytes.base642bytes(str, C64);
        assertThat(bytesWithC64, is(bytes));

        byte[] emptyBytes = Bytes.base642bytes("helium", 0, 0);
        assertThat(emptyBytes, is("".getBytes()));

        assertThat(Bytes.base642bytes("helium", 0, 0, ""), is("".getBytes()));
        assertThat(Bytes.base642bytes("helium", 0, 0, new char[0]), is("".getBytes()));
    }

    @Test
    public void testWrongBase64Code() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> Bytes.bytes2base64("helium".getBytes(), 0, 1, new char[]{'a'}));
    }

    @Test
    public void testWrongOffSet() {
        Assertions.assertThrows(IndexOutOfBoundsException.class, () -> Bytes.bytes2base64("helium".getBytes(), -1, 1));
    }

    @Test
    public void testLargeLength() {
        Assertions.assertThrows(IndexOutOfBoundsException.class, () -> Bytes.bytes2base64("helium".getBytes(), 0, 100000));
    }

    @Test
    public void testSmallLength() {
        Assertions.assertThrows(IndexOutOfBoundsException.class, () -> Bytes.bytes2base64("helium".getBytes(), 0, -1));
    }
