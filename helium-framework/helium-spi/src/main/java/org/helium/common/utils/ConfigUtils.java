...
public class ConfigUtils {
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty(CommonConstants.helium_PROPERTIES_KEY);
                    if (path == null || path.length() == 0) {
                        path = System.getenv(CommonConstants.helium_PROPERTIES_KEY);
                        if (path == null || path.length() == 0) {
                            path = CommonConstants.DEFAULT_helium_PROPERTIES;
                        }
                    }
     ...
     *
     * @param fileName       properties file name. for example: <code>helium.properties</code>, <code>METE-INF/conf/foo.properties</code>
    ...
    public static Properties loadProperties(String fileName, boolean allowMultiFile, boolean optional) {
        ...
        if (!allowMultiFile) {
            if (list.size() > 1) {
                String errMsg = String.format("only 1 %s file is expected, but %d helium.properties files found on class path: %s",
