...
/**
 * <b>描述:
 * </b>用于序列化组件，这是一个java.util.Date类型的解释器，在自动生成源码的过程中发现类某一个字段为java.util.Date类型时
 * ，会自动调用此解释器， 用于在创建源码时解释此字段如何序列化( write方法)、如何反序列化(parse方法)以及如何计算序列化长度
 * <p>
 * <b>功能: </b>java.util.Date类型的解释器,用于在创建源码时解释这个类型的字段字段如何序列化(
 * write方法)、如何反序列化(parse方法)以及如何计算序列化长度
 * <p>
 * <b>用法: </b>该类由序列化组件在遍历类中的字段时遇到java.util.Date类型时调用
 * <p>
 * 
 * @author Lv.Mingwei
 * 
 */
public class DateFieldInterpreter extends AbstractFieldInterpreter {
	 ...
	 * 		java.util.Date date = data.getData_obj();
	 ...
	 * 的输出
	 */
	private static final String WRITE_CODE_TEMPLATE = "if( ${data}.${getterName}() != null){ java.util.Date date = ${data}.${getterName}(); ${getGMTDate_code} Long millisecond = (date.getTime() + java.util.TimeZone.getDefault().getOffset(date.getTime())) * 10000;if (millisecond >= 2534022719990000000L)millisecond = 2534023007999999999L;else if (millisecond < -621355968000000000L)millisecond = -621355968000000000L;${output}.write${streamType}(${number}, millisecond);}";
	 ...
	 * 的输出
	 */
	private static final String SIZE_CODE_TEMPLATE = "if( ${data}.${getterName}() != null){ java.util.Date date = ${data}.${getterName}(); ${getGMTDate_code} Long millisecond = (date.getTime() + java.util.TimeZone.getDefault().getOffset(date.getTime())) * 10000;if (millisecond >= 2534022719990000000L)millisecond = 2534023007999999999L;else if (millisecond < -621355968000000000L)millisecond = -621355968000000000L;size += ${package_io}.CodedOutputStream.compute${streamType}Size(${number},millisecond);}";
	 ...
	 * 的输出
	 */
	private static final String PARSE_CODE_TEMPLATE = "long l = ${input}.read${streamType}();if (l > 2534022719990000000L)l = 2534022719990000000L / 10000;else if (l == -621355968000000000L) {l = -621357696000000000L;l = l / 10000 - java.util.TimeZone.getDefault().getOffset(l / 10000);} else {l = l / 10000 - java.util.TimeZone.getDefault().getOffset(l / 10000);}java.util.Date date = new java.util.Date(l); ${getGMTDate_code} ${data}.${setterName}(date);";
	 ...
	 * 的输出
	 */
	private static final String WRITE_CODE_ARRAY_TEMPLATE = " java.util.Date date = value; ${getGMTDate_code} Long millisecond = (date.getTime() + java.util.TimeZone.getDefault().getOffset(date.getTime())) * 10000;if (millisecond >= 2534022719990000000L)millisecond = 2534023007999999999L;else if (millisecond < -621355968000000000L)millisecond = -621355968000000000L;${output}.write${streamType}(${number}, millisecond);";
	 ...
	 * 的输出
	 */
	private static final String SIZE_CODE_ARRAY_TEMPLATE = "java.util.Date date = value; ${getGMTDate_code} Long millisecond = (date.getTime() + java.util.TimeZone.getDefault().getOffset(date.getTime())) * 10000;if (millisecond >= 2534022719990000000L)millisecond = 2534023007999999999L;else if (millisecond < -621355968000000000L)millisecond = -621355968000000000L;size += ${package_io}.CodedOutputStream.compute${streamType}Size(${number},millisecond);";
	 ...
	 * 的输出
	 */
	private static final String PARSE_CODE_ARRAY_TEMPLATE = "long l = ${input}.read${streamType}();if (l > 2534022719990000000L)l = 2534022719990000000L / 10000;else if (l == -621355968000000000L) {l = -621357696000000000L;l = l / 10000 - java.util.TimeZone.getDefault().getOffset(l / 10000);} else {l = l / 10000 - java.util.TimeZone.getDefault().getOffset(l / 10000);}java.util.Date date = new java.util.Date(l); ${getGMTDate_code} fieldValue = date;";
	...
	public synchronized static FieldInterpreter getInstance() {
		if (INSTANCE == null) {
			INSTANCE = new DateFieldInterpreter(ProtoFieldType.DATE);
		}
		return INSTANCE;
	}

	private DateFieldInterpreter(ProtoFieldType protoFieldType) {
	...
	public String getWriteCode(FieldInformation fieldInformation) {
		...
		if (fieldInformation.getAnnoField().timeZone() == TimeZone.UTC) {
			writeCode = writeCode.replaceAll("\\$\\{getGMTDate_code\\}", "date = " + Config.FEINNO_UTIL
					+ ".DateUtil.getGMTDate(date);");
		} else {
	...
	public String getSizeCode(FieldInformation fieldInformation) {
		...
		if (fieldInformation.getAnnoField().timeZone() == TimeZone.UTC) {
			sizeCode = sizeCode.replaceAll("\\$\\{getGMTDate_code\\}", "date = " + Config.FEINNO_UTIL
					+ ".DateUtil.getGMTDate(date);");
		} else {
	...
	public String getParseCode(FieldInformation fieldInformation) {
		...
		if (fieldInformation.getAnnoField().timeZone() == TimeZone.UTC) {
			parseCode = parseCode.replaceAll("\\$\\{getGMTDate_code\\}", "date = " + Config.FEINNO_UTIL
					+ ".DateUtil.getGMTDate(date);");
		} else {
