...
import org.helium.framework.spi.TaskReference;
...
public class BundleReference {
	...
	private Map<String, TaskReference> tasks;
		...
		for (BeanConfiguration bc: bundle.getBeans()) {
			switch (BeanType.fromText(bc.getType())) {
				...
				case SERVICE:
					...
					break;
				case TASK:
					TaskReference tr = tasks.get(bc.getId());
					if (tr == null) {
						tr = new TaskReference(bc, contextProvider);
						tr.setRouter(contextService.subscribeServerRouter(tr, bundleName, "rpc"));
						tasks.put(bc.getId(), tr);
						putBeanContext(tr);
					}
					break;
			}
	...
	private void putBeanContext(BeanContext ref) {
		...
		if (old == null) {
			...
			if ((old instanceof TaskInstance) && (ref instanceof TaskReference)) {
				((TaskInstance)old).combineReference((TaskReference)ref);
				LOGGER.info("Combine Task for {}", ref.getId());
			}
