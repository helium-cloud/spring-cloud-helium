...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        for (var i = 0; i < columns.length; i++) {
            if (columns[i].sortable) {
                th += '<th worker-name="' + columns[i].field + '">' + columns[i].fieldName + '<div class="btable-order"><div class="up" title="升序"><i class="fa fa-sort-asc" aria-hidden="true"></i></div ><div class="down" title="倒序"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div></th>';
            } else {
        ...
        tpl += '</table>';
        if (_config.paged) {
            tpl += '<div worker-type="paged" class="btable-paged"></div>';
        }
        tpl += '</div>';
        ...
        paging.init({
            ...
            pageConfig: { //分页参数配置
                skip: true,
                elem: $(_config.elem).find('div[worker-type=paged]'), //'#paged', //分页容器
                ...
                for (var i = 0; i < columns.length; i++) {
                    if (columns[i].format) {
                        $('#' + dataId).find('tr').each(function () {
                            var id = $(this).find('input[worker-item=id]').val();
                            var $field = $(this).children('td[worker-field=' + columns[i].field + ']');
                            var obj = undefined;
    ...
    function getTpl(options) {
        ...
        var tds = '';
        for (var i = 0; i < columns.length; i++) {
            tds += '<td worker-field="' + columns[i].field + '">{{ item.' + columns[i].field + ' }}</td>';
        }
        if (options.checkbox) {
            tds = '<td><input type="checkbox" worker-item="id" worker-id="{{ item.' + options.field + ' }}" lay-skin="primary" /></td><td>{{ (index+1) }}</td>' + tds;
        } else {
            tds = '<td style="display:none;"><input type="hidden" worker-item="id" value="{{ item.' + options.field + ' }}" worker-id="{{ item.' + options.field + ' }}" name="id" /></td><td>{{ (index+1) }}</td>' + tds;
        }
        tpl += '<tr>' + tds + '</tr>'
        tpl += '{{# }); }}';
        tpl += '{{# }else{ }}';
