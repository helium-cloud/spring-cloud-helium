...
layui.define(['element', 'common'], function (exports) {
	 ...
	 * @param {Object} data
	 */
    Tab.prototype.tabAdd = function (dataSource) {
        //对象与字符串之间转换
        var data = null;
        ...
        if (tabIndex === -1) {
            ...
            globalTabIdIndex++;
            var content = '<iframe src="' + data.href + '" worker-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
            ...
            if (_config.closed) {
                title += '<i class="layui-icon layui-unselect layui-tab-close" worker-id="' + globalTabIdIndex + '">&#x1006;</i>';
            }
            ...
            //iframe 自适应
            ELEM.contentBox.find('iframe[worker-id=' + globalTabIdIndex + ']').each(function () {
                $(this).height(ELEM.contentBox.height());
            });
            if (_config.closed) {
                //监听关闭事件
                ELEM.titleBox.find('li').children('i.layui-tab-close[worker-id=' + globalTabIdIndex + ']').on('click', function () {
            ...
            ELEM.contentBox.find('iframe[worker-id=' + globalTabIdIndex + ']').on('load', function () {
                //debugger;
                if (_config.openWait) {
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                var ul = '<ul>';
                ul += '<li worker-target="refresh" title="刷新当前选项卡"><i class="fa fa-refresh" aria-hidden="true"></i> 刷新</li>';
                ul += '<li worker-target="closeCurrent" title="关闭当前选项卡"><i class="fa fa-close" aria-hidden="true"></i> 关闭当前</li>';
                ul += '<li worker-target="closeOther" title="关闭其他选项卡"><i class="fa fa-window-close-o" aria-hidden="true"></i> 关闭其他</li>';
                ul += '<li worker-target="closeAll" title="关闭全部选项卡"><i class="fa fa-window-close-o" aria-hidden="true"></i> 全部关闭</li>';
                ul += '</ul>';
                div.innerHTML = ul;
                ...
                if ($context.length > 0) {
                    $context.eq(0).children('ul').children('li').each(function () {
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                case 'refresh': //刷新当前
                                    var src = ELEM.contentBox.find('iframe[worker-id=' + id + ']')[0].src;
                                    ELEM.contentBox.find('iframe[worker-id=' + id + ']')[0].src = src;
                                    break;
